...
public class AddingElementsIssueController extends AbstractController {
	Logger logger = LoggerFactory
			.getLogger(AddingElementsIssueController.class);
	...
	protected void performTask(HttpServletRequest request,
		...
		try {
			...
			request.setAttribute(ConstantsControllers.ASSIGNEES_LIST,
					assigneesList);

			IProjectDAO projectDAO = DAOFactory.getProjectDAOFromFactory();
			List<Project> projectsList = projectDAO.getObjectsList();
			request.setAttribute(ConstantsControllers.PROJECTS_LIST,
					projectsList);

			IBuildProjectDAO buildProjectDAO = DAOFactory
					.getBuildProjectDAOFromFactory();
			List<BuildProject> buildsProjectList = buildProjectDAO
					.getObjectsList();
			request.setAttribute(ConstantsControllers.BUILDS_PROJECT_LIST,
					buildsProjectList);
